...
У цьому розділі описано два основні сценарії використання, у яких Validator перевершує порівняно з найсучаснішими симуляціями SIL і HIL: розширене налагодження вбудованих систем і поступова реінжиніринг існуючих вбудованих систем, включаючи регресійне тестування. Приклади ілюструють кожен окремий сценарій використання валідатора.
...
Ключовою особливістю Validator, яка дозволяє розширене налагодження, є те, що в кожному рядку вихідного коду в завданнях контролера можна зупинити загальне моделювання, тобто як завдань контролера, так і заводу. Тоді змінні можна перевіряти та змінювати, зовнішній код можна виконувати тощо. Будь-який налагоджувач C можна приєднати до засобу перевірки для виконання загальних дій з налагодження завдань контролера. Сучасне середовище моделювання HIL не пропонує можливості налагодження. З іншого боку, неточність найсучасніших середовищ SIL робить налагодження непривабливим або принаймні менш корисним. Точність моделювання Validator робить налагодження цінним засобом перевірки та верифікації вбудованих систем. На малюнку 8.9 показано схематичне підключення відладчика до валідатора. Зображення реактивного винищувача представляє модель заводу.
...
Знімок екрана на [Малюнок 8.10] (#_bookmark51) показує зразок набору завдань контролера в Eclipse із додатком gnu debugger (gdb). Знімок екрана більш детально розглядається нижче. Ми використали gdb, оскільки він також підтримує зворотне (або історичне) налагодження. Це дозволяє наступне розширене налагодження: після ввімкнення зворотного налагодження налагоджувач записує всі зміни стану. Отже, якщо налагоджувач зупиняє виконання в точці зупинки, ви не можете лише зробити крок вперед, як зазвичай, але й зробити крок назад від цієї точки в коді. Наприклад, ви хочете знайти причину того, чому значення приводу перевищує певну межу. У цьому випадку ви повинні встановити умовну точку зупину, де встановлено значення приводу. Коли умова виконується, виконання на цьому зупиняється, і ви можете зробити крок назад крок за кроком.
...
Наступна послідовність скріншотів ілюструє зворотне налагодження з точки зору розробника з Eclipse і gdb як графічним інтерфейсом Validator.
...
У стані, показаному на малюнку 8.10, ми щойно увійшли в режим налагодження, натиснувши кнопку налагодження (помилка на панелі меню у верхній лівій частині вікна). Виконання зупинено на безумовній точці зупину у файлі dynamicsController.c. Інструкція в точці зупину — це інструкція присвоєння, у якій встановлюється значення змінної під назвою rtb_deg2rad2. Рядок виділено на вкладці dynamicsController.c. Відповідно до підвікна у верхній правій частині вікна, значення rtb_deg2rad2 дорівнює нулю.
...
Наступним кроком ми ввімкнемо зворотне налагодження, натиснувши відповідну кнопку (див. рис. 8.11). Під час кроку вперед панель керування налагодженням змінюється, щоб відобразити функцію зворотного налагодження, тобто можливість переходу вперед і назад (див. [Малюнок 8.12](#_bookmark52)).
...
Припустімо, що ми щойно зробили крок вперед на одне твердження (див. рис. 8.13). Оператор присвоєння, на якому ми спочатку зупинилися на точці зупину, очевидно, змінив значення змінної rtb_deg2rad2 з нуля на приблизно 1,807. Тепер ми можемо натиснути кнопку, щоб повернутися на один крок назад у процесі налагодження.
...
На малюнку 8.14 показано результат, тобто, як і очікувалося, значення змінної rtb_deg2rad2 знову дорівнює нулю. Зауважте, що симуляція дискретних подій валідатора була реалізована таким чином, що зворотне налагодження також функціонує для кількох завдань. Наприклад, якщо ви ввімкнули зворотне налагодження, крок вперед, і завдання переривається іншим завданням, тобто симуляція перемикається на інше завдання, ви все одно можете зробити крок назад до точки, з якої почали зворотне налагодження.
...
Як майбутнє розширення до Validator, ми додамо функцію, щоб також мати можливість встановлювати точки зупинки в симуляції заводу, які зупиняють загальну симуляцію.
...
Початковою мотивацією для розробки валідатора було забезпечення надійної підтримки для поступової міграції системи контролера двигуна (ECS) великого виробника автомобілів до версії, в якій поведінка синхронізації явно моделюється за допомогою мови визначення часу (TDL) [9]. ,10]. З цією метою поведінку застарілої ECS і ECS на основі TDL слід детально порівняти в симуляції SIL, тобто якомога ближче до поведінки на фактичній платформі. На малюнку 8.15 показано загальне налаштування для цього типу регресійного тестування за допомогою прикладу ECS. Валідатор може імітувати обидві версії паралельно.
...
Зауважте, що ECS містить мільйони рядків коду, здебільшого написаного мовою C, і працює на основі операційної системи OSEK. Це вимагало ефективної реалізації моделювання дискретних подій валідатора та ефективного спільного моделювання з моделлю автомобільного двигуна (= заводу). Модель двигуна представлена в MATLAB і Simulink. Щоб точно зафіксувати час подій кута кривошипа, модель двигуна моделюється за допомогою розв’язувача змінного кроку.
...
Реалізація семантики TDL у оновленому ECS [9] вимагала виділеного компонента TDL під назвою *TDL-Machine*, який мав виконуватися кожні 0,5 мс від завдання з найвищим пріоритетом у системі. TDL-машина використовувала додаткові глобальні змінні для зберігання та відновлення значень вихідних глобальних змінних у певні моменти часу.
...
Найвищий пріоритет завдання в оригінальному ECS мав період 1 мс. Щоб уникнути введення нового завдання, було вирішено змінити вихідне завдання на період 0,5 мс, викликати TDL-Machine під час кожного виклику завдання та виконувати вихідний код завдання кожного другого виклику. Таким чином, період виконання вихідного коду не вплинув.
...
Давайте подивимося на один із результатів регресійних тестів: на малюнку 8.16 показано вибірку з трьох сигналів, що відстежуються під час моделювання двох версій ECS за допомогою Validator. Сигнали S1 і S2 схожі в обох версіях. Можна помітити деякі затримки, внесені в модифіковану версію через час виконання TDL-Machine. Сигнал S3 значно відрізняється до кінця змодельованого періоду часу.

The cause of the difference could be found by an investigation of task execution profiles. Task execution profile plots are shown in Figures 8.17a through c. In each plot, every task is assigned an identification number (ID). The execution state of a task with ID *i* is represented by a signal with *y*-coordinates between *i* and *i* + 0.6. A level of *i* + 0.6 indicates the execution mode E, a level of *i* + 0.4 means preempted mode P, a level of *i* + 0.2 indicates the waiting mode W, and a level of *i* means the task is in the suspended mode S. The plot in Figure 8.17a is obtained from the simulation of the original software, the plot in Figure 8.17b is obtained from the simulation of the software with the TDL-Machine and its execution time, and the plot in Figure 8.17c corresponds to the simulation of the software with the TDL-Machine but without its execution time. In case of Figure 8.17c, the TDL-Machine was executed in simulation, but its execution time was set to zero, for the purpose of debugging.

Причину різниці можна знайти шляхом дослідження профілів виконання завдань. Графіки профілів виконання завдання показані на малюнках 8.17a–c. На кожній ділянці кожному завданню присвоюється ідентифікаційний номер (ID). Стан виконання завдання з ID *i* представлено сигналом з *y*-координатами між *i* та *i* + 0,6. Рівень *i* + 0,6 вказує на режим виконання E, рівень *i* + 0,4 означає вимкнений режим P, рівень *i* + 0,2 вказує на режим очікування W, а рівень *i* означає завдання знаходиться в режимі призупинення S. Графік на малюнку 8.17a отримано з моделювання оригінального програмного забезпечення, графік на малюнку 8.17b отримано з моделювання програмного забезпечення за допомогою TDL-машини та часу його виконання, а графік на малюнку 8.17c відповідає моделюванню програмного забезпечення за допомогою TDL-Machine, але без часу його виконання. У випадку, зображеному на малюнку 8.17c, TDL-машина була виконана в симуляції, але час її виконання було встановлено на нуль з метою налагодження.
...
Ідентифікатор завдання з найвищим пріоритетом дорівнює 8. У моделюванні, представленому діаграмою на малюнку 8.17a, час виконання цього завдання в момент часу 5 становить приблизно 0,05 мс.
...
**РИСУНОК 8.17** (a) Оригінальний ECS. (b) ECS з додатковою функціональністю. (c) ECS з додатковою функціональністю, час виконання якого встановлено на нуль.
...
При моделюванні діаграми на малюнку 8.17b те саме виконання вимагає приблизно 0,20 мс. На малюнку 8.17c виконання того самого завдання займає приблизно 0,17 мс. Звідси випливає, що основна різниця між виконаннями на малюнках 8.17a і 8.17b не визначається часом виконання TDL-Machine, а тому, що інший стан апаратної платформи є результатом виконання функції TDL. Оскільки TDL-машина виконує багато звернень до нових ділянок пам'яті, головна відмінність виникає, швидше за все, у стані кешу. Якщо затримки сигналів у новій версії неприйнятні, то слід зосередитися на мінімізації впливу TDL-Machine на стан платформи, а не на мінімізації часу виконання коду функції. Наприклад, TDL-машину можна змінити так, щоб вона працювала лише з локальними змінними, або додаткові змінні, що використовуються цією функцією, можуть зберігатися у внутрішньому просторі пам’яті процесора.
